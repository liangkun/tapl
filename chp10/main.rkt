...
;; Chapter 10
;; Implementation of typed lambda calculas.


;; Assertion
(define (assert condition format-str . objs)
  (if (not condition)
      (error (apply format format-str objs))
      (void)))

;; Type Definitions
(struct Type ())
(struct TyBool Type ())
(struct TyFun (tyarg tyresult))
...
(struct TmAbs Term (tmbody name tyarg))
(struct TmApp Term (tmabs tmarg))

;; Variable context implementation.
;; We use a list of (string . type) pairs to store variable infos currently.
;; Note: variable indexes are used as the static distances in chapter 6.

;; This is the internal structure of ctx.
(struct CtxImpl (length varinfos))

;; Create a varinfo from a name and a type
(define varinfo cons)

;; Get name from a varinfo
(define varname car)

;; Get type from a varinfo
(define vartype cdr)

;; Create a new ctx from a list of names and types.
;; If the types is '() list, set all types to #f (unknown type).
(define (make-ctx names (types '()))
  (if (null? types)
      (CtxImpl (length names) (map (lambda (name) (cons name #f)) names))
      (CtxImpl (length names) (map varinfo names types))))

;; Get length of the context.
(define (ctxlength ctx) (CtxImpl-length ctx))

;; Get all the varinfos in the context.
(define (ctxvarinfos ctx) (CtxImpl-varinfos ctx))

;; Get the varinfo at the given index
(define (index->varinfo ctx index)
  (assert (and (>= index 0) (< index (ctxlength ctx)))
          "ctxlength: ~a, index: ~a." (ctxlength ctx) index)
  (list-ref (ctxvarinfos ctx) index))

;; Get all the names in the context.
(define (ctxnames ctx) (map varname (ctxvarinfos ctx)))

;; Get the name on the index.
(define (index->name ctx index) (varname (index->varinfo ctx index)))

;; Get the type on the index.
(define (index->type ctx index) (vartype (index->varinfo ctx index)))

;; Get the index of the name, returns false if no such name in the ctx.
(define (name->index ctx name)
  (let loop ([index 0] [names (ctxnames ctx)])
    (cond
      [(null? names) #f]
      [(string=? name (car names)) index]
      [else (loop (+ index 1) (cdr names))])))

;; Shifts the context by inserting a new varinfo at index 0.
(define (ctxshiftin ctx info)
  (CtxImpl (+ (ctxlength ctx) 1) (cons vinfo (ctxvarinfos ctx))))

;; Append the new varinfo after the context.
(define (ctxappend ctx name (type #f))
  (CtxImpl (+ (ctxlength ctx) 1) (append (ctxvarinfos ctx) (varinfo name type))))

;; Merge two ctx into one.
(define (ctxmerge master slave)
  (let loop ([ctx master] [infos (varinfos slave)])
    (if (null? infos)
        ctx
        (let ([info (car infos)] [infos (cdr infos)])
          (if (name->index ctx (varname info))
              (loop ctx infos)
              (loop (ctxshiftin ctx info) infos))))))
  ...
  (match term
    [(TmVar fi index length)
     (assert (= length (ctxlength ctx))
             "variable at ~a has a wrong ctx expect: ~a, got: ~a"
    ...
    (match term
      [(AstVar fi name)
       (cond
         ...
         [else (assert #f "Should not reach here: ~a\n" term)])]
